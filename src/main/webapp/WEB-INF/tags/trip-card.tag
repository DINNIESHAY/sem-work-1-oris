<%@ tag description="Trip Post Component" pageEncoding="UTF-8" %>
...
<form action="${contextPath}/trip-info" method="get" class="trip-card">
    ...
    <div class="trip-header">
        ...
        <div class="header-actions">
            ...
            <!-- лайк -->
            <c:if test="${not empty user and tripWithOwner.userInfo.userId != user.id}">
                <c:choose>
                    <c:when test="${tripWithOwner.favorite}">
                        <a href="${contextPath}/remove-from-favorites?tripId=${tripWithOwner.trip.id}&returnUrl=trips-feed"
                           class="favorite-btn favorite-active" title="Удалить из избранного">
                            <i class="fas fa-heart"></i>
                        </a>
                    </c:when>
                    <c:otherwise>
                        <a href="${contextPath}/add-to-favorites?tripId=${tripWithOwner.trip.id}&returnUrl=trips-feed"
